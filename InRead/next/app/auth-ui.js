...
const copy = {
  en: {
    eyebrow: "Reading, not word hoarding", title: "Read the book in front of you.", lead: "InRead finds the few words that block one real book, then sends you back to the story.", zoomHint: "If any interface is obscured, use your browser zoom to adjust the page to a comfortable size.", login: "Log in", register: "Create account", enter: "Enter InRead", account: "My account", admin: "Admin console"
  },
  zh: {
    eyebrow: "不背海量词，只读眼前书", title: "在真实阅读中，自然习得。", lead: "InRead 只找出真正卡住当前这本书的少量词汇，再把你送回故事里。", zoomHint: "如遇界面遮挡，可使用浏览器缩放功能，将网页调整至合适大小。", login: "登录", register: "注册", enter: "进入 InRead", account: "我的账户", admin: "管理后台"
  }
};
...
function UserMenu({ user }) {
  ...
  const hasPlan = (state.plan?.flatMap((day) => day.tasks || []).length || 0) > 0;
  const continueReadHref = hasBook ? `/reader?book=${encodeURIComponent(state.selectedBook.id)}` : "/legacy/search.html";
  const continueStudyHref = hasPlan
    ? (state.studySession && !state.studySession.completed ? "/legacy/study.html" : "/legacy/plan.html")
    : "/legacy/search.html";
  const planHref = hasPlan ? "/legacy/plan.html" : "/legacy/search.html";
  const zh = typeof window !== "undefined" && localStorage.getItem("inread-language") === "zh";
  const text = zh
    ? { recent: "最近学习", emptyBook: "还没有开始一本书", words: "已掌握", start: "从一本想读的书开始", read: "继续阅读", study: "继续背词", plan: "当前训练计划", logout: "退出登录", admin: "管理后台" }
    : { recent: "Recent learning", emptyBook: "No book started yet", words: "words mastered", start: "Start with a book you want to read", read: "Continue reading", study: "Continue vocabulary study", plan: "Current study plan", logout: "Log out", admin: "Admin console" };
  async function logout() { await api("/api/auth/logout", { method: "POST" }); window.location.href = "/"; }
  return <div className="user-menu"><a className="account-chip" href="/account"><span className="mini-avatar">{user.avatar}</span><span>{user.nickname}</span></a><div className="user-popover"><p className="popover-label">{text.recent}</p><strong>{state.selectedBook?.title || text.emptyBook}</strong><span>{learned ? `${learned} ${text.words}` : text.start}</span><a className="secondary-action" href={continueReadHref}>{text.read}</a><a className="secondary-action" href={continueStudyHref}>{text.study}</a><a className="secondary-action" href={planHref}>{text.plan}</a>{user.role === "admin" && <a className="plain-button admin-popover-link" href="/admin">{text.admin}</a>}<button className="logout-button" onClick={logout}>{text.logout}</button></div></div>;
}
...
export function Landing() {
  ...
  return <main className="auth-page landing-page">
    <nav className="marketing-nav"><a className="wordmark" href="/">InRead</a><div className="nav-actions"><ThemeControl /><button className="plain-button" onClick={() => setLanguage(language === "en" ? "zh" : "en")}>{language === "en" ? "中文" : "EN"}</button>{user ? <UserMenu user={user} /> : <><a href="/login">{t.login}</a><a className="dark-link" href="/register">{t.register}</a></>}</div></nav>
    <section className="landing-hero"><p className="auth-eyebrow">{t.eyebrow}</p><h1>InRead</h1><p className="origin-line">不背海量词，只读眼前书</p><p>{t.lead}</p><div className="hero-actions">{user ? <><a className="primary-action" href="/legacy/search.html">{language === "zh" ? "继续阅读" : "Continue reading"}</a><a className="secondary-action" href="/legacy/plan.html">{language === "zh" ? "查看已创建的学习计划" : "View my study plan"}</a></> : <><a className="primary-action" href="/login">{language === "zh" ? "开始" : "Start"}</a><a className="secondary-action" href="/register">{t.register}</a></>}</div><p className="viewport-hint">{t.zoomHint}</p></section>
