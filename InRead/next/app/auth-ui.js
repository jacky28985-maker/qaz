...
export function Landing() {
  ...
  return <main className="auth-page landing-page">
    <nav className="marketing-nav"><a className="wordmark" href="/">InRead</a><div className="nav-actions"><button className="plain-button" onClick={() => setLanguage(language === "en" ? "zh" : "en")}>{language === "en" ? "中文" : "EN"}</button>{user ? <UserMenu user={user} /> : <><a href="/login">{t.login}</a><a className="dark-link" href="/register">{t.register}</a></>}</div></nav>
    <section className="landing-hero"><p className="auth-eyebrow">{t.eyebrow}</p><h1>InRead</h1><p className="origin-line">不背海量词，只读眼前书</p><p>{t.lead}</p><div className="hero-actions"><a className="primary-action" href={user ? "/legacy/search.html" : "/login"}>{user ? t.enter : (language === "zh" ? "开始" : "Start")}</a>{!user && <a className="secondary-action" href="/register">{t.register}</a>}</div></section>
...
export function LoginForm() {
  ...
  async function submit(event) { event.preventDefault(); const form = new FormData(event.currentTarget); try { await api("/api/auth/login", { method: "POST", body: JSON.stringify({ account: form.get("account"), password: form.get("password") }) }); window.location.href = "/"; } catch (error) { setMessage(error.message); } }
  return <AuthShell title="欢迎回来" subtitle="登录后继续你自己的阅读路径。"><form onSubmit={submit} className="auth-form"><label>账号<input name="account" autoComplete="username" required /></label><label>密码<input name="password" type="password" autoComplete="current-password" required /></label>{message && <p className="form-error">{message}</p>}<button className="primary-action">登录</button><a className="forgot-link" href="/forgot-password">我忘记了密码</a><p>还没有账号？ <a href="/register">去注册</a></p></form></AuthShell>;
}

export function RegisterForm() {
  const [message, setMessage] = useState("");
  async function submit(event) { event.preventDefault(); const form = new FormData(event.currentTarget); try { await api("/api/auth/register", { method: "POST", body: JSON.stringify({ account: form.get("account"), nickname: form.get("nickname"), password: form.get("password"), inviteCode: form.get("inviteCode") }) }); setMessage("注册成功，请登录。"); event.currentTarget.reset(); } catch (error) { setMessage(error.message); } }
...
function AuthShell({ title, subtitle, children }) { return <main className="auth-page"><nav className="marketing-nav"><a className="wordmark" href="/">InRead</a><a href="/">返回首页</a></nav><section className="auth-card"><p className="auth-eyebrow">INREAD ACCOUNT</p><h1>{title}</h1><p className="auth-subtitle">{subtitle}</p>{children}</section></main>; }
